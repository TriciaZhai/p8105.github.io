...
Download and install the latest versions of R (3.5.1) and RStudio (1.1.456). 

Install several packages (and their dependencies) using the code below:

```{r eval = FALSE}
install.packages(c("tidyverse", "janitor", "broom", "here", "shiny", "readxl", "haven", "ggridges", "viridis", "skimr", "rvest", "httr", "flexdashboard", "devtools", "rmarkdown", "googlesheets", "modelr"))
```

Upload a screenshot of RStudio thats show you can successfully load the packages `tidyverse` and `knitr`, includes , and include your UNI on the command line. Here's an example "solution":
