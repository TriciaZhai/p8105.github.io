...
An obvious place to start is to consider price as an outcome that may depend on rating and boro. We fit that initial model in the following code. 
...
R will treat categorical (factor) covariates appropriately and predictably: indicator variables are created for each non-reference category and included in your model, and the factor level is treated as the reference. As with `ggplot`, being in control of your factors is therefore critical!
...
It's important to note that changing reference categories won't change "fit" or statistical sigificance, but can affect interpretation.
...
The reason that we omitted the output is because it's a huge pain to deal with. `summary` produces an object of class `summary.lm`, which is also a list -- that's how we got to the coefficients. `coef` produces a vector of coefficient values, and `fitted.values` is a vector of fitted values. None of this is tidy. 
...
Of course, these can be included in pipeline of commands to conduct inspections and perform diagnostics.
...
This example has some obvious issues, most notably the presence of extremely large outliers in price and a generally skewed distribution. There are a few things we might try to do here -- including creating a formal rule for the exclusion of outliers, transforming the price variable (e.g. using a log transformation), or fitting a model that is robust to outliers. Dealing with these issues isn't really the purpose, though, so we'll note the issues and move on; shortly we'll look at using the [bootstrap](bootstrapping.html) for inference in cases like this, where standard approaches to inference may fail. 

(For what it's worth, I'd probably use a combination of median regression, which is less sensitive to outliers than OLS, and bootstrapping for inference. If that's not feasible, I'd omit rentals with price over $1000 (< 0.5% of the sample) from the primary analysis and examine these separately. I usually avoid transforming the outcome, because models become difficult to interpret.)
...
The test of interest is implemented in the `anova` function which, of course, summarized using `broom::tidy`. 
...
The estimates here are consistent with those in the model containing interactions, but are easier to identify.
